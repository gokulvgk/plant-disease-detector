...
# Define disease prevention advice
  ...
  # Replace with your key or load from environment
advice_dict = {}
def get_local_phi3_advice(disease):

    try:
        # Start ollama phi3 in background
        subprocess.Popen(["ollama", "run", "phi3"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        time.sleep(5)  # Wait a few seconds to let the model load
    except Exception as e:
        print(f"Failed to start phi3: {e}")
        return False
    prompt = f"Provide prevention and treatment advice for {disease} in plants in short answer."
...
def predict_disease(image_path):
    ...
    prevention_advice = advice_dict.get(disease_name, get_local_phi3_advice(disease_name))
